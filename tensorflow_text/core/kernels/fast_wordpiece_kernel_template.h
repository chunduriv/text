...
#ifndef THIRD_PARTY_TENSORFLOW_TEXT_CORE_KERNELS_FAST_WORDPIECE_KERNEL_H_
#define THIRD_PARTY_TENSORFLOW_TEXT_CORE_KERNELS_FAST_WORDPIECE_KERNEL_H_
...
absl::Status FastWordpieceTokenizeWithOffsetsOp<Rt>::ShapeInference(
  ...
  /*if (!input_values_shape.Compatible(rank_1_shape)) {
    return absl::FailedPreconditionError("Shape must be rank 1: ");
  }
...
absl::Status FastWordpieceDetokenizeOp<Rt>::ShapeInference(
  ...
  >>> row_splits = [0, 3, 5]
  >>> tokens = fast_wordpiece_detokenize(tokens, row_splits, model_buffer)
  >>> tokens
  ['a abc', 'abcd']
  ```
...
#endif  // THIRD_PARTY_TENSORFLOW_TEXT_CORE_KERNELS_FAST_WORDPIECE_KERNEL_H_
